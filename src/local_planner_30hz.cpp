...
//  PlanarVehicleSimulator (prediction never diverges from execution).
// ────────────────────────────────────────────────────────────────────
...
uint64_t LocalPlanner30Hz::commandTieHash(double vx, double vy, double yr) {
    // FNV-1a over the IEEE bit patterns.  All three values are finite
    // (clamped) at call time, so the memcpy is well-defined.  This is only
    // the final deterministic fallback after side-neutral geometric and
    // command-change comparisons; it never reads a LEFT/RIGHT label.
...
void LocalPlanner30Hz::reset() {
    ...
    // Per-mission continuity bookkeeping (v5).
    last_mission_revision_ = 0;
    last_target_position_ = Vec2d(0.0, 0.0);
    last_target_valid_ = false;
    // Clear all stall / control-oscillation detector state.
...
// ────────────────────────────────────────────────────────────────────
//  Every mode (ordinary lattice candidates, TURN_TO_TARGET, the terminal
//  feedback controller and the emergency brake) outputs a command that is
//  reachable from the CURRENT state within one control period under the
//  configured acceleration / yaw-acceleration limits.  This is THE single
//  definition of the executable output; the simulator and the expert
//  labels only ever see these values.
...
bool LocalPlanner30Hz::updateMissionState(const LocalTarget& target,
    ...
    // A mission_revision change (new task / goal revision / macro enter /
    // macro exit) always resets planner memory.
    if (target.mission_revision != last_mission_revision_) reset_memory = true;
    // A large target jump (> target_discontinuity_reset_m) is a
    // discontinuity and ALSO resets planner memory.
...
BodyCommand2D LocalPlanner30Hz::terminalIntent(
    ...
    // Stop around the centre half of the accepted goal disk.  The
    // proportional bound supplies a continuous approach speed while the
    // braking bound guarantees that the feedback rollout can settle without
    // overshooting it at the configured acceleration limit.
    ...
    // Once inside the accepted goal disk, settle residual rotation instead
    // of chasing a geometrically ill-conditioned bearing near the goal
    // centre.  Outside it, keep facing the target but at a gentle rate.
...
LocalPlannerCandidate LocalPlanner30Hz::makeTerminalCandidate(
    ...
    // The terminal controller is also an expert-label source: its INTENT is
    // the raw feedback law, and its executable output goes through the same
    // one-control-period reachable window as every lattice candidate.
    ...
    candidate.stable_index = -1;  // preferred only by the explicit terminal path

    // Roll out the same feedback law that will be recomputed at 30 Hz.  A
    // constant terminal command would incorrectly predict motion through
    // and beyond the goal, potentially checking irrelevant obstacles behind
    // it and reintroducing the very overshoot this mode is meant to remove.
    Trajectory2D traj;
...
PlannerResult LocalPlanner30Hz::computePlan(const VehicleState2D& state,
    ...
    // Static macro-handoff base clearance (see handoffClearance()).  Filled
    // on EVERY path (turn / success / failure) so consumers can always read
    // the planner's envelope without mistaking a missing value for a
    // measurement.
    res.handoff_clearance_m = handoffClearance();
    // ── AUDIT: the 30 Hz planner never touches truth / global data. ──
    // (fields remain false by construction — no such data is reachable)
    ...
    if (!target.valid) {
        ...
        if (mutate) {
            ...
            // Track the mission anyway so a later valid target with a new
            // revision is treated as a fresh mission.
            updateMissionState(target, res);
        }
        return res;
    }

    // ── Per-mission continuity / discontinuity bookkeeping (mutate only) ──
    // A mission_revision change or a > target_discontinuity_reset_m target
    // jump resets planner memory (command continuity + stall window).  A
    // plain update_event change NEVER resets anything (rolling 5 Hz targets).
    if (mutate) {
        updateMissionState(target, res);
    }
    ...
    // Near-goal heading relaxation: within lp_near_goal_heading_relax_distance
    // the ENTER threshold is widened (default 75°) so the vehicle decelerates
    // and converges instead of re-spinning around the goal; a target clearly
    // behind (> widened threshold) still forces a turn.
    ...
    // clamped to max_yaw_rate and the shared kinematics limit yaw accel.
    // Inside the accepted goal disk, translational/yaw settling has priority.
    // The bearing becomes ill-conditioned near the goal centre and must not
    // re-enter a rotate-in-place loop.
    if (dist <= p_.task_goal_tolerance) {
        turn_hysteresis_active_ = false;
    ...
    if (turn_hysteresis_active_) {
        // TURN_TO_TARGET uses the SAME output semantic as every other mode:
        // the yaw-rate intent (turn_k * bearing, clamped to max_yaw_rate) is
        // projected through the one-control-period yaw-acceleration window
        // via reachableCommand — never output raw.
        res.success = true;
        res.turn_mode = true;
        ...
        if (mutate) {
            ...
            last_command_ = out;
            has_last_command_ = true;
            // A turn can last many real ticks while this detector receives
            // no samples.  Do not join stale pre-turn samples to post-turn
            // samples and report a false short-window oscillation.
            limit_cycle_window_.clear();
            limit_cycle_detected_ = false;
            last_cycle_mission_revision_ = target.mission_revision;
        }
    ...
    //    Uses the SAME speed-dependent dynamic envelope as the candidates:
    //    the un-executed suffix is re-checked with its local velocity and
    //    the closing-speed reaction+braking required clearance, so a newly
    //    observed obstacle triggers immediate avoidance BEFORE the vehicle
    //    reaches the 0.5 m hard boundary.
    if (mutate) {
    ...
    // Does an observed obstacle currently intersect the vehicle →
    // LocalTarget corridor?  Pure diagnostic + gating information: the
    // avoidance itself is driven continuously by the per-candidate
    // obstacle-risk cost (computeObstacleRisk), never by a distance gate.
    {
    ...
    // Accept one candidate with an explicit category.  vx_body/vy_body/
    // yaw_rate are the EXECUTABLE OUTPUT (one-control-period reachable
    // command); intent_* are the rollout intent.
    auto acceptCandidate = [&](const LocalPlannerCandidate& selected,
        ...
        // Soft-clearance / dynamic-envelope diagnostics: the ACTUAL
        // measured distance (never a small-radius inf) and the max
        // dynamic requirement / closing speed along the candidate.
    ...
    // Collect the full spatial risk neighbourhood ONCE per 30 Hz tick. The
    // smaller dynamic-clearance neighbourhood remains candidate-local.
    ...
    // It still passes through evaluateCandidate(), so an observed obstacle,
    // UNKNOWN cell or FOV violation causes a normal fallback to avoidance.
    // v7: the terminal controller is applied UNIFORMLY to every LocalTarget
    // (final goal, 5 Hz macro guide, rolling target-network output) — the
    // 30 Hz planner never branches on the target source.
    ...
    // Inside it a SAFE_HOLD (zero-speed) candidate is allowed: the vehicle
    // is settling, not stalled.  Away from it a stop candidate can NEVER be
    // selected while a progressing candidate exists.  This is a pure
    // DISTANCE rule applied uniformly to every target source.
    const bool terminal_region = dist <= p_.lp_terminal_control_distance;

    // 2) Generate + evaluate candidates.
    // NOTE: the container must be MUTABLE — evaluateCandidate() takes
    // LocalPlannerCandidate& and writes reject_reason/cost/feasible.
    auto candidates = generateCandidates(state);
    // v5: dynamic_window_candidate_count = number of INTENT candidates that
    // actually enter the rollout (joint intent+output dedup keeps every
    // distinct long-term intent, so intents are never collapsed by the
    // one-period reachable window).
    ...
    // Deterministic best-candidate rule (WITHIN one category): primary =
    // cost (with tie tolerance); on a tie prefer smaller terminal heading
    // error, smaller cross-track, smaller |vy_body|, smaller |yaw_rate|,
    // smaller command yaw-rate change, then the side-neutral tie hash.
    // LEFT/RIGHT never appear as explicit semantic preferences.  The
    // category hierarchy (progressing vs hold) is applied OUTSIDE this
    // comparator.
    auto betterThan = [&](const LocalPlannerCandidate& a,
        ...
        // Command continuity compares the last EXECUTABLE output; it stays
        // active across small continuous LocalTarget moves (mission
        // unchanged) and is only broken by a mission/discontinuity reset.
        ...
        // Final deterministic fallback over command values (not the sample
        // enumeration index and not a LEFT/RIGHT semantic label).
    ...
    for (auto& c : candidates) {
        ...
        if (!ok) {
            // Per-tick rejection diagnostics: each rejected candidate counts
            // its FIRST decisive reason exactly once.
            countRejected(reject_enum);
    ...
    // 1) hard safety (only feasible candidates reach here);
    // 2) valid progress: away from the goal ONLY a SAFE_PROGRESSING
    //    candidate may be selected — a safe stop can never beat it;
    // 3) in the terminal convergence region a safe hold is allowed
    //    (TERMINAL_SETTLING);
    // 4) within the category, tracking/clearance/smoothness costs decide.
    if (best_progressing || (terminal_region && best_safe)) {
        ...
        // ── Stall / control-oscillation detector (mutate only) ──────
        // A detected limit cycle (physically stuck + blockage/oscillation
        // evidence) escalates to a safe brake + BLOCKED classification in
        // EVERY target mode — there is no is_macro_guide special case, so
        // the same local observation/state/target always produces the same
        // expert action.  The macro layer advances the guide (v7) so a
        // transient brake cannot become a self-sustaining deadlock.
        if (mutate) {
            ...
            if (cycle) {
                ...
                // Preserve candidate intent/progress diagnostics, but make
                // the final-output contract explicit and self-consistent.
                res.output_progress_qualified = false;
                res.progress_qualified = false;
    ...
    // 3) Failure: classify + safe braking.  NEVER report success here.
    // A real observed blockage is decided FIRST from the per-candidate
    // rejection reasons (hard 0.5 m OR the dynamic braking envelope).
    // The old "target-direction centreline 0.5 m" check alone is no longer
    // sufficient — a dynamic-envelope rejection must not be mislabelled
    // NO_SAFE_CANDIDATE, or the 5 Hz expert would never trigger.
    res.success = false;
    ...
    if (best_safe) {
        // Safe candidates exist but none has valid target progress (away
        // from the goal).  With observed-blockage evidence this is a real
        // blockage (macro-eligible); without evidence it is a 30 Hz-internal
        // stall that must never be success and must never be masked by the
        // 5 Hz privileged expert (open-space success+stop regression).
        ...
        // Hold the best safe candidate's reachable command (known-free).
    ...
    // Emergency brake: stopping distance not available.  The output still
    // goes through the shared one-control-period reachable window.
    ...
    if (mutate) {
        ...
        has_last_command_ = true;
        // Record this failure sample and let the detector fire on
        // persistent real blockage (never on UNKNOWN-driven samples, which
        // carry no blocked evidence).
...
PreviewResult LocalPlanner30Hz::previewPlan(const VehicleState2D& state,
    ...
    // v5: macro exit requires a SAFE AND PROGRESSING trajectory — a safe
    // stop alone never qualifies.
...
bool LocalPlanner30Hz::currentTrajectoryBlocked(
    ...
    if (!current_trajectory_.valid || current_trajectory_.points.empty())
        return false;
    // Find the trajectory point nearest to the CURRENT vehicle position;
    // only the executable SUFFIX after it is re-checked (already-passed
    // segments are never re-examined).  Segments are sampled continuously.
    size_t start_i = 0;
    ...
    for (size_t i = start_i + 1; i < current_trajectory_.points.size(); ++i) {
        ...
        // Suffix local velocity (the trajectory's own prediction), so the
        // dynamic envelope reacts to the speed actually planned here.
...
std::vector<LocalPlannerCandidate> LocalPlanner30Hz::generateCandidates(
    ...
    // ── v5 candidate semantics ─────────────────────────────────────
    // speed_samples / lateral_ratio_samples / yaw_rate_samples are LONG-TERM
    // control INTENTS.  Each intent is:
    //   * rolled out for the FULL 2.5 s horizon (simulateCandidate applies
    //     the shared kinematics accel/yaw-accel/speed limits per lp_dt, so
    //     the vehicle accelerates from standstill toward the intent); and
    //   * paired with an EXECUTABLE OUTPUT = reachableCommand(state,
    //     intent) — the one-control-period reachable command that is
    //     actually sent to the simulator / recorded as the expert label.
    // The old behaviour projected each raw sample into the tiny one-period
    // window and then rolled out THAT tiny command for 2.5 s, collapsing
    // every distinct long-term speed into "stop / barely moving".
    struct Raw {
    ...
    for (double vx : p_.lp_speed_samples) {
        for (double lr : p_.lp_lateral_ratio_samples) {
            // Body frame +Y = left (CCW): positive ratio → LEFT, negative
            // → RIGHT; the sample set is left/right symmetric.
    ...
    // Different long-term intents that project to the SAME reachable first
    // command are KEPT (their rollouts differ), so the one-period window can
    // never collapse distinct speed intentions.  Only an identical intent
    ...
    // LEFT/RIGHT mirror INTENTS (vx, +vy, +yr) / (vx, -vy, -yr) share the
    // same index, so the index itself can never encode a side preference.
    ...
    for (const Raw& r : ordered) {
        ...
        // Final deterministic tie-break hashes the INTENT (side-neutral;
        // the output is a deterministic function of intent + state, so the
        // hash uniquely identifies the deduped candidate).
        cand.tie_hash = commandTieHash(r.desired_vx, r.desired_vy, r.desired_yr);
        // Nominal rollout with the LONG-TERM INTENT via the shared
        // kinematics (prediction never diverges from execution).
...
bool LocalPlanner30Hz::evaluateCandidate(LocalPlannerCandidate& c,
    ...
    // ── Soft-clearance / dynamic-envelope accumulators (v4) ────────
    ...
    // `is_vehicle_point` only skips the FOV bearing test for the exact
    // current pose (its bearing is undefined); the zero-length start point
    // is the sensor origin, which is FREE by contract (see FovRaycaster2D
    // sensor-origin marking) — the first NON-zero trajectory point onward
    // is always fully validated.  UNKNOWN is never treated as free.
    // `v_seg` is the segment's local velocity (used for the closing-speed
    // dynamic envelope).
    auto pointOk = [&](const Vec2d& p, const Vec2d& v_seg,
        ...
        if (!is_vehicle_point) {
            ...
            // A zero-length sample (stationary candidate) coincides with
            // the current pose — its bearing is undefined; atan2(0,0)-yaw
            // would spuriously fail whenever the world yaw is outside the
            // FOV.  Only real motion is FOV-tested; known-free and
            // clearance still apply to the pose itself.
            if (rel.squaredNorm() > 1e-12) {
                ...
                // FOV constraint: the executable prefix must stay inside the
                // CURRENT sensor wedge (history may contain cells outside it).
        ...
        // Known-FREE requirement: UNKNOWN is never free space.
        if (!obs.isKnownFree(p.x(), p.y())) {
            why = "not_known_free";
            why_enum = CandidateRejectReason::NOT_KNOWN_FREE;
            return false;
        }
        // Soft + dynamic clearance.  Search radius is at least the soft
        // clearance radius, widened when the dynamic envelope needs more
        // (computed with the TOTAL segment speed — an upper bound of the
        // closing speed, so the query never misses a relevant obstacle).
        // Evaluate the dynamic envelope against every observed occupied
        // cell in the bounded disk (pre-collected, distance-filtered).
        // Looking only at the nearest cell can miss a slightly farther
        // obstacle that lies directly in the velocity direction while the
        // nearest cell is merely tangential.
        const double search_r = clearanceSearchRadius(v_seg.norm());
    ...
    // The 2.5 s prediction is only executable up to the first sample that
    // leaves the CURRENT FOV or enters UNKNOWN. An observed-clearance
    // violation rejects the whole candidate (see below). We
    // walk the swept segments and grow `prefix_last` only while a WHOLE
    // segment is safe (samples at ≤ obs_resolution/2).  This fixes the
    // second start-up deadlock: a candidate whose far end swings outside
    // the current FOV is no longer rejected wholesale — its safe prefix is
    // executed and scored instead.  Segment endpoints are validated once
    // per segment boundary (k=1..steps); the shared endpoints of adjacent
    // segments are not re-sampled, but no segment interior is skipped.
    size_t prefix_last = 0;  // index of last fully-safe trajectory point
    ...
    for (size_t i = 0; i + 1 < c.traj.points.size() && !failed; ++i) {
        ...
        // Segment-local velocity for the closing-speed dynamic envelope.
        double dt_seg = p_.lp_dt;
        ...
        // k = 0 is point `a`: the vehicle point (sensor origin, FREE by
        // contract) or the previous segment's end (already validated).
        for (int k = 1; k <= steps; ++k) {
            const Vec2d p = a + (b - a) * (static_cast<double>(k) / steps);
            std::string why;
            CandidateRejectReason why_enum = CandidateRejectReason::NONE;
            // pointOk updates the selected-prefix diagnostics.  If this
            // point is the first unsafe one, it is outside the executable
            // prefix and must not contaminate its min-clearance / dynamic
            // requirement metrics.
    ...
    // A visible occupied-clearance violation (hard 0.5 m OR the dynamic
    // braking envelope) rejects the whole candidate in EVERY target mode:
    // the retained short prefix would point at the blocker and creep
    // forward, and the same local observation/state/target must always
    // yield the same expert action.  The macro layer guarantees (v7) that
    // the guide is far enough ahead and its chord is clearance-certified,
    // so a safe prefix toward the guide is not spuriously rejected here.
    if (failed &&
    ...
    // A candidate is only executable if its safe prefix covers at least
    // lp_min_executable_prefix_s (≥ lp_dt, so the command held until the
    // next 30 Hz replan is guaranteed safe).
    ...
    // Build the EXECUTABLE prefix trajectory (what the vehicle will
    // actually follow until the next replan).
    Trajectory2D exec;
    exec.valid = true;
    ...
    // only be selected inside the terminal region.
    ...
    // Unified normaliser: max_speed × scoring_horizon, the SAME time scale
    // for every candidate (never the total distance to the target).
    const double achievable_progress_m = p_.lp_max_speed * scoring_horizon;

    // ── Progress qualification (v5) ────────────────────────────────
    // A candidate "has valid target-direction progress" only when the
    // EXECUTABLE OUTPUT actually moves (>= min_progress_speed_mps) AND it
    // gains at least min_progress_epsilon_m toward the LocalTarget over its
    // certified prefix.  A safe stop never qualifies away from the goal.
    ...
    // The reference line is built ONLY from the current vehicle position
    // and the current LocalTarget (never a global path, never privileged).
    // It is re-created every tick from the latest LocalTarget, so macro
    // guides (continuously changing local targets) are supported naturally.
    ...
    // Use the CURRENT local reference direction for every candidate.  The
    // former end->target direction was abruptly disabled as soon as a
    // candidate endpoint entered goal_tolerance (even while the vehicle was
    // still well outside it), which removed all directional preference and
    // caused lateral command chatter.  A current->target reference remains
    // continuous under a rolling LocalTarget and never exposes a global path.
    ...
    // End velocity direction (finite difference over the last prefix step).
    // Near-zero speed has an undefined atan2(0,0) → fall back to heading.
    ...
    // Cross-track: mean |lateral offset| of the prefix points from the
    // local reference line, normalised by lp_cross_track_normalize_m.
    ...
    // ── Cost terms (all normalised; formulas documented in README) ──
    // progress is normalised by achievable_progress = max_speed *
    // scoring_horizon (a UNIFIED time scale for every candidate), NOT by
    // the total distance to the target.  A certified prefix shorter than
    // the scoring horizon receives an explicit, continuous truncation
    // penalty, so a short / stationary trajectory can never gain an unfair
    // "fully observed for 2.5 s" advantage.
    ...
    // ── CONTINUOUS SOFT CLEARANCE COST (v4) ─────────────────────────
    // Old formula (1 - cl/min_clearance) was zero for EVERY feasible
    // candidate once cl >= 0.5 m, so there was no early avoidance
    // gradient.  New formula is a continuous piecewise ramp:
    //   hard_clearance = lp_min_clearance (0.5 m)
    //   soft_radius    = lp_soft_clearance_radius_m (2.0 m)
    //   no obstacle found in the search radius  → 0
    //   otherwise:
    //     cost_clearance = clamp((soft_radius - min_clear) /
    //                            max(eps, soft_radius - hard_clearance),
    //                            0, 1)
    // Clearance near 2.0 m → ~0; approaching 0.5 m → ~1; strictly between
    // → a continuous gradient that makes feasible candidates steer away
    // from obstacles BEFORE the hard 0.5 m gate fires.
    ...
    // Smoothness = acceleration-change (jerk) proxy only, computed on the
    // EXECUTABLE OUTPUT commands and the certified prefix.  Use three
    // consecutive velocity samples to form two accelerations; a two-
    // velocity expression measures acceleration, not acceleration change.
    ...
    // Adjacent-command continuity is keyed on the MISSION revision: it
    // stays active across small continuous LocalTarget moves (a plain
    // update_event change never cancels smoothness) and is only broken by
    // a mission change / discontinuity reset / reset().
    ...
    // Include the acceleration discontinuity at the candidate boundary.
    // Previously a 2-3 point prefix executed no jerk loop at all and got a
    // free zero smoothness cost, even after a violent command reversal.
    ...
    // Legacy diagnostic name `speed_change`, now comparing only ADJACENT
    // EXECUTABLE OUTPUT commands (never the long-term intent speed).  A
    // normal acceleration from standstill toward a fast intent produces a
    // tiny one-period output step and is NOT penalised; only a genuine
    // output-command change (lateral reversal / re-acceleration after a
    // stop) costs.  "Smaller speed" is never implicitly treated as
    // "smoother".
    ...
    // Yaw-rate CHANGE cost (replaces the old absolute-turning cost): penalise
    // (a) the commanded yaw rate relative to the CURRENT yaw rate (stick
    // yank) and (b) mean yaw acceleration inside the prefix (high-frequency
    // turning / oscillation).  Legitimate heading correction towards the
    // target is NOT penalised here — alignment is rewarded by the
    // terminal_heading / velocity_alignment terms.
    ...
    // The ACTUAL measured soft-clearance distance (inf only when NO
    // OCCUPIED cell was found inside the soft/dynamic search radius — the
    // search is wide enough that a nearby obstacle is never missed).
    ...
    c.traj = std::move(exec);  // the executable (truncated) trajectory
    c.feasible = true;
    return true;
}
...
void LocalPlanner30Hz::computeObstacleRisk(
    ...
    // Use the long-term intent direction.  The first rollout displacement
    // is dominated by current velocity and made all small lateral choices
    // look identical just when a distant blocker first became visible.
    ...
    for (const Vec2d& cell : occ_cells) {
        ...
        if (lon <= 0.0) continue;  // behind the vehicle along this direction
        const double lat = std::fabs(cross2(dir_cand, to));
        // Longitudinal gate: the obstacle counts up to the horizon, then the
        // risk decays smoothly over a shoulder (no hard binary edge).
        const double lon_gate = clamp(
            (lon_horizon + lon_shoulder - lon) /
                std::max(1e-6, lon_shoulder),
            0.0, 1.0);
        if (lon_gate <= 0.0) continue;
        // Corridor membership (soft): 1 inside the half-width, decays over
        // a shoulder so an obstacle just beside the corridor contributes
        // less but not with a binary jump.
        const double g_corridor = clamp(
            1.0 - (lat - corridor_hw) / std::max(1e-6, corridor_shoulder),
            0.0, 1.0);
        // Closest approach of this cell to the candidate's nominal rollout.
        ...
        // Trajectory-sweep gate: off-corridor obstacles still count when the
        // predicted path sweeps near them (the second avoidance condition).
        ...
        // Geometric distance factor along the candidate direction.
        const double dist_factor = clamp(
            (lon_horizon - lon) / std::max(1e-6, lon_horizon - hard),
            0.0, 1.0);
        // Closest-approach factor: grows as the predicted pass gets tighter.
        const double clear_factor = clamp(
            (clear_horizon - d_path) /
                std::max(1e-6, clear_horizon - hard),
            0.0, 1.0);
        // TTC is the first predicted entry into the desired-clearance
        // shell.  The old closest-point time incorrectly made a stationary
        // rollout report TTC=0 even when the obstacle was metres away.
...
bool LocalPlanner30Hz::corridorBlockedByObserved(
    ...
    // March from the vehicle toward the target and look for OCCUPIED
    // cells within min_clearance of the corridor.
    const Vec2d to = target.position - state.position;
    const double dist = to.norm();
    ...
    // The target cell itself being occupied also counts as real blockage.
...
bool LocalPlanner30Hz::spaceToStop(const VehicleState2D& state,
                                   const LocalObservation& obs,
                                   double dist) const {
    // Braking corridor direction: use the actual velocity direction when
    // moving (the vehicle may have significant lateral velocity), and fall
    // back to the yaw-forward direction only when nearly stopped.
    const double v = state.velocity_world.norm();
    Vec2d dir;
    ...
    // Check the swept DRONE-CENTRE trajectory.  The configured safety and
    // handoff clearances are already centre-to-obstacle-surface distances,
    // so adding lateral body offsets here would double-inflate them.
    // UNKNOWN is never guaranteed safe.
    const double step = p_.obs_resolution * 0.5;
    ...
    // scene_safety_clearance is defined from the drone centre to the
    // obstacle surface.  Sample the centre trajectory directly; applying a
    // lateral drone-radius offset and then the full centre clearance would
    // double-inflate the geometry.  Always validate the current pose too,
    // including the already-stationary (dist == 0) case.
    // The current pose only needs to satisfy the hard collision-safety
    // contract.  Requiring the larger macro handoff clearance here creates a
    // deadlock: a vehicle that has safely entered the [hard, handoff) shell
    // cannot even validate a zero-distance stop and is forced into a
    // permanent EMERGENCY_BRAKE.  The handoff/dynamic envelope applies to
    // the swept motion that follows the current pose.
...
// ────────────────────────────────────────────────────────────────────
//  Deterministic stall / control-oscillation detector (v5, §7)
// ────────────────────────────────────────────────────────────────────
//  Maintains a short ring window of the last `lp_limit_cycle_window_ticks`
//  30 Hz samples (mission revision, distance to target, selected EXECUTABLE
//  command, observed/dynamic blockage, position, target position).  The
//  window is cleared on reset(), on a MISSION revision change (which the
//  FSM bumps on new tasks, final-goal changes, macro enter and macro exit)
//  and on a target discontinuity reset — NEVER on a plain update_event
//  change, so the detector keeps working for a continuously moving 5 Hz
//  target network.  There is no is_macro_guide branch in the detector.
//  previewPlan never calls this.
//
//  "Stuck" metric: PHYSICAL vehicle displacement over the window (robust to
//  a moving LocalTarget — distance-to-target change is not meaningful when
//  the target itself rolls) plus progress projected along each per-tick
//  target direction.  Fires when the vehicle is stuck AND either
//    A) real observed / dynamic-clearance blockage evidence on at least
//       lp_limit_cycle_min_blocked_ticks ticks of the window, or
//    B) repeated "near-zero command → aggressive lateral → near-zero
//       command" patterns / lateral command sign reversals.
//
//  On firing the caller must command a safe brake and classify the failure
...
bool LocalPlanner30Hz::updateLimitCycle(const PlannerResult& res,
                                        const LocalTarget& target,
                                        const VehicleState2D& state) {
    // Clear the window on a mission change, a target discontinuity reset or
    // a macro-guide flag change (fresh observation window).  A plain
    // update_event change NEVER clears it (continuous targets).
    if (limit_cycle_window_.empty()) {
        last_cycle_mission_revision_ = target.mission_revision;
    } else if (last_cycle_mission_revision_ != target.mission_revision ||
               res.target_discontinuity_reset) {
        // New mission / macro entry / macro exit / discontinuity: fresh
        // window.  A plain update_event change NEVER clears it (continuous
        // rolling targets keep the detector armed).
        limit_cycle_window_.clear();
        limit_cycle_detected_ = false;
        last_cycle_mission_revision_ = target.mission_revision;
    }
    ...
    // Only REAL observed / dynamic-clearance blockage counts as evidence;
    // UNKNOWN-driven NO_SAFE_CANDIDATE samples carry no blockage evidence.
    // Blockage evidence means the FINAL plan outcome could not execute
    // progress.  Some rejected lattice samples are normal around every
    // visible obstacle and must never turn an otherwise successful escape
    // command into persistent-blockage evidence.
    ...
    // Need a reasonably filled window (at least half) to draw conclusions.
    ...
    // ── Stuck metric 1: PHYSICAL vehicle displacement ──────────────
    // A moving target never hides a stationary vehicle.
    const double displacement = (last.position - first.position).norm();
    // ── Stuck metric 2: progress projected along each per-tick target
    //    direction (sum of positive projections of the per-step vehicle
    //    displacement on the direction to the target AT THAT SAMPLE). ──
    ...
    if (displacement >= p_.lp_limit_cycle_net_progress_m ||
        ...
        // The vehicle actually moved (or made forward progress): no stall.
        limit_cycle_detected_ = false;
        return false;
    }

    // ── Condition A: persistent real blockage + stuck ──────────────
    int blocked_ticks = 0;
    ...
    // ── Condition B: control oscillation ────────────────────────────
    // "near-zero command → aggressive lateral → near-zero command"
    // patterns and lateral command sign reversals with no movement.
    int sign_flips = 0;
    ...
    // Without observed obstacle evidence this is a tracking/controller
    // issue, not grounds for invoking the privileged obstacle expert.
    if (blocked_ticks > 0 &&
